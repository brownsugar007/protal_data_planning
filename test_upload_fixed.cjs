...
const rows = jsonData.filter(row => {
    ...
    return dbCols.map(col => {
        ...
        if (col === 'date' && v instanceof Date) {
            // Fix Excel precision bug (e.g. 23:59:24 instead of 00:00:00) by adding 12 hours
            const fixedDate = new Date(v.getTime() + 12 * 60 * 60 * 1000);
...
if (rows.length > 0) {
    ...
    // print the last 5 days distribution
